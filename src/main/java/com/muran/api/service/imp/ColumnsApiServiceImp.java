...
public class ColumnsApiServiceImp extends AbstractService implements
    ...
    @Override
    public Response getColumnConfigList() {
        return Response.ok().entity(configDao.findAll()).build();
    }

    @Override
    @Transactional
    public Response updateColumnConfig(Long autoId, Boolean bool) {
        ColumnConfig columnConfig = configDao.findOne(autoId);
        if (columnConfig == null) {
            throw new ServerException(Code.BadRequestParams, "栏目配置信息不存在！");
        }
        columnConfig.setShowReadNum(bool);
        columnConfig = configDao.update(columnConfig);
        return Response.ok().entity(columnConfig).build();
    }
        ...
        for (ColumnItem columnItem : listParent) {
            ...
            if (listChild!=null&&listChild.size()>0) {
                ...
                for (ColumnItem columnItem2 : listChild) {
                    ...
                    if (columnItem2.getType().equals("click")){
                        subButton.setKey(columnItem2.getUrl());
                    }else if (columnItem2.getType().equals("view")){
